...
namespace PharmacyTool.Web.Code.SilverlightFileUpload
{
    ...
    public class FileUploadProcess
    {
        ...
        public void ProcessRequest(HttpContext context, string uploadPath)
        {
            ...
            string filename = context.Request.QueryString["filename"].Replace('@', '\\'); // @を\に戻す
        ...
        private void DirectoryCheck(string Filepath)
        {
            FileInfo fi = new FileInfo(Filepath);
            DirectoryInfo di = fi.Directory;
            Stack<string> staDirNames = new Stack<string>();
            string CreateDirectoryRoot = "";
            ...
            while (true)
            {
                ...
                staDirNames.Push(di.Name);
                di = di.Parent;
                CreateDirectoryRoot = di.FullName;
            }


            foreach (var dirName in staDirNames)
            {
                Directory.CreateDirectory(Path.Combine(CreateDirectoryRoot,dirName));
                CreateDirectoryRoot = Path.Combine(CreateDirectoryRoot,dirName);
            }
        }
    }
}
